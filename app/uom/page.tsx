...
export default function UOMPage() {
  ...
  const create = async () => {
    ...
    await fetch("/api/uom", { method: "POST", body: JSON.stringify({ name, description: desc }) });
    setName(""); setDesc(""); load();
  };

  const update = async () => {
    if (!selectedId) return alert("Select a row first");
    await fetch("/api/uom", { method: "PUT", body: JSON.stringify({ id: selectedId, name, description: desc }) });
    load();
  };

  const remove = async () => {
    if (!selectedId) return alert("Select a row first");
    await fetch("/api/uom", { method: "DELETE", body: JSON.stringify({ id: selectedId }) });
    setSelectedId(null);
    setName(""); setDesc("");
    load();
  };
